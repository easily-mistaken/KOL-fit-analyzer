...
The audience layout is **brand-aware** (Unit 42). `non_crypto` is the only bucket defined by negation, so on its own it answers nothing — and for a brand that isn't crypto it inverts, since that residual is their entire market while the chart spends its slices on `defi_users`/`meme_degens`. So: a crypto-native brand keeps one "Outside crypto" slice that carries the domain breakdown as tooltip/legend **members** (no extra slice, no extra hue); a non-crypto brand gets that slice opened into real domain slices, with the crypto-only buckets folded into one slice labelled **"Crypto-native"** — named, not dumped into "Other", because a nameless residual is the exact failure being fixed. Role buckets (founders, developers, investors, community managers, creators) stay visible for both. `OrgClassification.cryptoNative` picks the layout; absent means crypto-native.

Domain slices introduce **no new colour**. The non-crypto layout folds the six crypto-only buckets and replaces `non_crypto`, freeing seven validated hues, and the domains likeliest to carry share take those — so a domain can't collide with anything else on screen. The rare tail (science, news, culture) reuses a role bucket's hue, bounded by the 6-slice cap and by every slice carrying a swatch + label + percentage. "Unclear" deliberately shares the "Other" grey: to a reader they are the same idea, and one grey family beats a third grey nobody can distinguish.
