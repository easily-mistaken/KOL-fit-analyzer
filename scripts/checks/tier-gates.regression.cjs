...
// --- defaults ------------------------------------------------------------------
ck("defaults: 3 anonymous / 12 authenticated", TIER_LIMITS.anonLifetime === 3 && TIER_LIMITS.userLifetime === 12);
...
ck("user 3 -> allowed (claimed history counts, 9 left)", decideTier(3, true).allowed === true);
ck("user 11 -> allowed (12th report)", decideTier(11, true).allowed === true);
ck("user 12 -> upgrade wall", decideTier(12, true).allowed === false && decideTier(12, true).gate === "upgrade_required");
...
ck("invalid overrides fall back to defaults", bad.anonLifetime === 3 && bad.userLifetime === 12);
